            ...
            while time.time() - start_time < timeout:
                try:
                    if self.platform == 'Windows':
                        # Windows: use msvcrt for advisory locking
                        try:
                            import msvcrt
                            ...
                            # Advisory locking not available - continue anyway
                            pass
                    else:
                        ...
                        try:
                            ...
                            try:
                                ...
                                # If fcntl.flock fails, just continue
                                pass
                        except (ImportError, OSError):
                            # fcntl not available - continue anyway
                            pass
                    
                    # If we get here, lock was acquired or is not required
                    break
...
def _normalize_path(path: Union[str, Path]) -> Path:
    """Normalize path to ensure it's relative and properly formatted.
    ...
    # Convert to string for absolute path checking
    path_str = str(path)
...
class FileLock:
    ...
    def _platform_has_locking(self) -> bool:
        """Check if the platform supports file locking."""
        if self.platform == 'Windows':
            try:
                import msvcrt
                return True
